...
app = FastAPI(title="Webhook Pagos Rifa", version="4.1.0-supabase-fallback")
...
def columnas_auditoria() -> list:
    return ["Fecha_Hora", "Proveedor", "Evento", "External_Reference", "Payment_ID", "Session_ID", "Monto", "Moneda", "Resultado", "Mensaje", "Request_ID", "Data_ID"]
...
def append_worksheet(nombre: str, filas: List[Dict[str, Any]], columnas: list) -> None:
    ...
    ejecutar_con_reintento(ws.append_rows, df.astype("object").where(pd.notna(df), "").values.tolist(), value_input_option="USER_ENTERED")
...
def registrar_auditoria_sheets(proveedor: str, evento: str, external_reference: str = "", payment_id: str = "", session_id: str = "", monto: Any = "", moneda: str = "", resultado: str = "", mensaje: str = "", request_id: str = "", data_id: str = "") -> None:
    if not COPIAR_A_SHEETS or not SPREADSHEET_ID:
        return
    try:
        append_worksheet("Auditoria", [{"Fecha_Hora": ahora_txt(), "Proveedor": proveedor, "Evento": evento, "External_Reference": external_reference, "Payment_ID": payment_id, "Session_ID": session_id, "Monto": monto, "Moneda": moneda, "Resultado": resultado, "Mensaje": str(mensaje)[:500], "Request_ID": request_id, "Data_ID": data_id}], columnas_auditoria())
    except Exception:
        pass
...
def extraer_data_id_mp(payload: Dict[str, Any], request: Request) -> str:
    ...
    opciones = [request.query_params.get("data.id", ""), request.query_params.get("id", ""), request.query_params.get("data_id", ""), data.get("id", "") if isinstance(data, dict) else "", payload.get("id", "")]
...
def validar_firma_mercadopago(payload: Dict[str, Any], request: Request) -> str:
    ...
    firma_calculada = hmac.new(MP_WEBHOOK_SECRET.encode("utf-8"), manifest.encode("utf-8"), hashlib.sha256).hexdigest()
...
def debe_reconstruir_reserva_supabase(error: Exception) -> bool:
    texto = str(error).lower()
    return any(p in texto for p in ["no hay reservas pendientes", "no existe reserva", "reservas pendientes", "external_reference"])
...
def crear_reserva_supabase_desde_sheets(external_reference: str) -> Dict[str, Any]:
    ...
    params = {"p_external_reference": ext_ref, "p_boletos": boletos, "p_nombre": nombre, "p_correo": correo, "p_telefono": telefono, "p_precio": precio, "p_minutos_reserva": int(os.getenv("SUPABASE_RESERVA_MINUTOS_FALLBACK", "1440"))}
...
def confirmar_pago_en_supabase(payment_info: Dict[str, Any], payload_resumen: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    params = {"p_provider": proveedor, "p_external_reference": ext_ref, "p_payment_id": payment_id, "p_session_id": session_id, "p_preference_id": preference_id, "p_monto_pagado": obtener_monto_pagado(payment_info), "p_moneda": obtener_moneda_pago(payment_info), "p_metodo_pago": limpiar_valor(payment_info.get("payment_type_id", proveedor.lower())), "p_evento": DEFAULT_EVENTO, "p_payload": payload_resumen or {}}
    try:
...
def copiar_pago_a_sheets(payment_info: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    with SHEETS_LOCK:
        ...
        for _, r in grupo.iterrows():
            nuevas.append({"ID_Boleto": f"BOL-{random.randint(10000, 99999)}", "Nombre": r.get("Nombre", ""), "Correo": r.get("Correo", ""), "Evento": DEFAULT_EVENTO, "Numero_Boleto": parse_ticket_number(r.get("Numero_Boleto", "")), "Precio": r.get("Monto", ""), "Metodo_Pago": metodo_pago, "Codigo_Pago": pago_id, "Fecha_Compra": ahora_txt(), "Numero_Telefonico": r.get("Numero_Telefonico", ""), "Estado_Pago": "VENDIDO", "Referencia_Pago": ext_ref, "MercadoPago_Payment_ID": pago_id if proveedor == "MERCADO_PAGO" else "", "MercadoPago_Preference_ID": mp_preference_id if proveedor == "MERCADO_PAGO" else r.get("MercadoPago_Preference_ID", ""), "Stripe_Payment_ID": pago_id if proveedor == "STRIPE" else "", "Stripe_Session_ID": stripe_session_id if proveedor == "STRIPE" else "", "Proveedor_Pago": proveedor})
...
def procesar_pago_confirmado(payment_info: Dict[str, Any], evento: str, payload_resumen: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if COPIAR_A_SHEETS and SPREADSHEET_ID:
        try:
            ...
            sheets_error = str(e)
    registrar_auditoria_sheets(proveedor, evento, external_reference=ext_ref, payment_id=pago_id, session_id=session_id, monto=monto, moneda=moneda, resultado="OK", mensaje=f"Supabase OK. Sheets={len(sheets_result)}. {sheets_error}"[:500])
    return {"ok": True, "provider": proveedor, "supabase": supabase_result, "sheets_ventas": len(sheets_result), "sheets_error": sheets_error}
...
def payment_info_desde_checkout_session(session: Any) -> Optional[Dict[str, Any]]:
    ...
    return {"id": limpiar_valor(payment_intent), "external_reference": limpiar_valor(metadata.get("external_reference") or safe_get(s, "client_reference_id", "")), "payment_type_id": "stripe_card", "provider": "STRIPE", "stripe_session_id": limpiar_valor(safe_get(s, "id", "")), "amount_total": safe_get(s, "amount_total", 0), "currency": limpiar_valor(safe_get(s, "currency", DEFAULT_CURRENCY_STRIPE)).lower()}
...
def payment_info_desde_payment_intent(intent: Any) -> Optional[Dict[str, Any]]:
    ...
    return {"id": limpiar_valor(safe_get(i, "id", "")), "external_reference": limpiar_valor(metadata.get("external_reference", "")), "payment_type_id": "stripe_card", "provider": "STRIPE", "stripe_session_id": "", "amount_total": safe_get(i, "amount", 0), "currency": limpiar_valor(safe_get(i, "currency", DEFAULT_CURRENCY_STRIPE)).lower()}
...
async def webhook_stripe(request: Request):
    ...
    try:
        ...
        if event_type in ["checkout.session.completed", "checkout.session.async_payment_succeeded"]:
            info = payment_info_desde_checkout_session(data_obj)
        elif event_type == "payment_intent.succeeded":
            info = payment_info_desde_payment_intent(data_obj)
        if info and info.get("external_reference"):
            return JSONResponse(procesar_pago_confirmado(info, event_type, safe_to_dict(data_obj)))
...
def payment_info_desde_mp(pago: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if not pago or pago.get("status") != "approved":
        return None
    return {"id": limpiar_valor(pago.get("id", "")), "external_reference": limpiar_valor(pago.get("external_reference", "")), "payment_type_id": limpiar_valor(pago.get("payment_type_id", "mercado_pago")), "provider": "MERCADO_PAGO", "mp_preference_id": limpiar_valor(pago.get("preference_id", "")), "transaction_amount": pago.get("transaction_amount", 0), "currency_id": limpiar_valor(pago.get("currency_id", DEFAULT_CURRENCY_MP)).upper()}
...
@app.get("/health")
def health():
    return {"ok": True, "spreadsheet_configurado": bool(SPREADSHEET_ID), "copiar_a_sheets": COPIAR_A_SHEETS, "fallback_reserva_desde_sheets": SUPABASE_FALLBACK_SHEETS, "stripe_configurado": bool(STRIPE_SECRET_KEY), "stripe_webhook_secret_configurado": bool(STRIPE_WEBHOOK_SECRET), "mp_configurado": bool(MP_ACCESS_TOKEN), "mp_webhook_secret_configurado": bool(MP_WEBHOOK_SECRET), "supabase_url_configurado": bool(SUPABASE_URL), "supabase_service_role_configurado": bool(SUPABASE_SERVICE_ROLE_KEY), "supabase_cliente_activo": supabase is not None, "total_boletos": TOTAL_BOLETOS, "digitos_boleto": DIGITOS_BOLETO}

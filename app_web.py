...
def sincronizar_total_boletos_supabase() -> None:
    ...
    try:
        resultado = supabase.rpc(
            "inicializar_boletos",
            {"p_total_boletos": total_app, "p_reiniciar_movimientos": reiniciar}
        ).execute()
...
def cancelar_boleto_reserva_supabase(external_reference: str, numero_boleto: str, motivo: str = "CANCELADO_USUARIO") -> Tuple[bool, str]:
    ...
    try:
        ...
        supabase.table("reservas").update({"estado_reserva": motivo, "fecha_actualizacion": ahora_iso}).eq("external_reference", ref).eq("numero_boleto", boleto).eq("estado_reserva", "PENDIENTE").execute()
        supabase.table("boletos").update({"estado": "DISPONIBLE", "external_reference_actual": None, "fecha_actualizacion": ahora_iso}).eq("numero_boleto", boleto).eq("estado", "RESERVADO").eq("external_reference_actual", ref).execute()
        limpiar_cache_mapa()
...
def cancelar_reserva_supabase_por_referencia(external_reference: str, motivo: str = "CANCELADO_USUARIO") -> Tuple[bool, str]:
    ...
    try:
        ...
        ahora_iso = datetime.now().isoformat()
        if boletos:
            supabase.table("reservas").update({"estado_reserva": motivo, "fecha_actualizacion": ahora_iso}).eq("external_reference", ref).eq("estado_reserva", "PENDIENTE").execute()
...
def consultar_ventas_supabase_por_boleto_correo(numero_boleto: str, correo: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not data:
            return []
        return obtener_ventas_supabase_por_referencia(data[0].get("external_reference", ""))
    except Exception:
        return []
...
def liberar_reserva_y_mantener_seleccion(external_reference: str) -> Tuple[bool, str]:
    """
    Cuando la persona abandona Stripe/Mercado Pago sin pagar:
    - Libera la reserva real en Supabase para que no quede como RESERVADO.
    - Conserva los boletos en selected_tickets como seleccion local.
    - Conserva checkout_pendiente con nombre, correo, telefono y boletos.
    - El mapa muestra los boletos como seleccionables/propios, no bloqueados.
    """
    ref = limpiar_valor_id(external_reference)
    checkout = st.session_state.get("checkout_pendiente")
    ...
    if not boletos:
        boletos = [
            parse_ticket_number(b)
            for b in st.session_state.get("selected_tickets", [])
            if parse_ticket_number(b)
        ]

    if ref:
        cancelar_reserva_supabase_por_referencia(ref, motivo="CANCELADO_USUARIO")
    ...
    st.session_state.selected_tickets = boletos

    if isinstance(checkout, dict):
        checkout["boletos"] = boletos
        checkout["total"] = float(PRECIO_BOLETO) * len(boletos)
        st.session_state.checkout_pendiente = checkout
    ...
    st.session_state.stripe_session_id = None
    st.session_state.payment_provider = None
    st.session_state.external_ref_activa = ref if ref else st.session_state.get("external_ref_activa", None)

    limpiar_cache_mapa()
    return True, "Reserva liberada y seleccion conservada."
...
def renderizar_mapa_interactivo():
    ...
    selected_validos = len([t for t in st.session_state.selected_tickets if t not in estados_bd])
    libres = max(0, TOTAL_BOLETOS - vendidos - reservados - otros - selected_validos)
    st.markdown(f"""
...
def sincronizar_reserva_checkout_en_sheets(conn: GSheetsConnection) -> Tuple[bool, str]:
    ...
    if supabase_activo():
        ...
        for boleto in boletos_checkout:
            ordenes_supabase.append({"External_Reference": ref, "MercadoPago_Preference_ID": "", "MercadoPago_Payment_ID": "", "Stripe_Session_ID": "", "Stripe_Payment_ID": "", "Numero_Boleto": boleto, "Nombre": nombre_completo, "Correo": correo, "Numero_Telefonico": telefono, "Monto": float(PRECIO_BOLETO), "Estado_Reserva": "PENDIENTE", "Fecha_Creacion": ahora_txt, "Expira_En": expira_txt, "Fecha_Actualizacion": ahora_txt})
    ...
    for boleto in boletos_nuevos:
        nuevas.append({"External_Reference": ref, "MercadoPago_Preference_ID": "", "MercadoPago_Payment_ID": "", "Stripe_Session_ID": "", "Stripe_Payment_ID": "", "Numero_Boleto": boleto, "Nombre": nombre_completo, "Correo": correo, "Numero_Telefonico": telefono, "Monto": float(PRECIO_BOLETO), "Estado_Reserva": "PENDIENTE", "Fecha_Creacion": ahora_txt, "Expira_En": expira_txt, "Fecha_Actualizacion": ahora_txt})
...
def renderizar_checkout_pendiente(conn: GSheetsConnection):
    ...
    if not metodo:
        ...
        st.info(
            ...
            "Una vez confirmado el pago, los cambios, cancelaciones o reembolsos se revisarán de manera puntual para brindar la mejor solución en caso de errores, esto durante el evento.\n\n"
...
def main():
    ...
    inicializar_estado()
    sincronizar_total_boletos_supabase()
    conn = st.connection("gsheets", type=GSheetsConnection)
